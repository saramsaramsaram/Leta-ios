...
class ChatViewModel: ObservableObject {
    ...
    private func handleUnauthorized(context: String, statusCode: Int) {
        print("❌ [\(context)] 인증 실패 HTTP \(statusCode). 저장된 토큰을 삭제하고 다시 로그인을 요청합니다.")
            ...
            do {
                ...
                guard let sendBody = try? JSONEncoder().encode(sendReq),
                      var streamRequest = NetworkManager.shared.createRequest(
                        urlPath: "/api/chat/room/\(roomRes.id)/stream/send",
                        method: "POST",
                        body: sendBody,
                        requireAuth: true
                      ) else {
                ...
                for try await line in streamBytes.lines {
                    ...
                    if trimmedLine.hasPrefix("data:") {
                        let raw = trimmedLine.replacingOccurrences(of: "data:", with: "")
                                             .trimmingCharacters(in: .whitespaces)
                        if raw == "[DONE]" {
                            print("🏁 [채팅] 스트리밍 완료 ([DONE] 수신)")
                            break
                        }
                        token = raw
                    } else {
                ...
                print("⚠️ [채팅] 스트리밍 통신 중 에러 발생: \(error)")
            }
